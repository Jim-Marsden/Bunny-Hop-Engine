...
std::shared_ptr<sf::Texture> TextureManager::MakeTexture(const std::string &string) {
    AddTexture(string);
    return textures[string];
}

std::shared_ptr<sf::Texture> TextureManager::CGetTexture(const std::string &string) const {
    if(textures.at(string) != nullptr) return textures.at(string);
    return nullptr;

}

bool TextureManager::CheckTexture(const std::string &texture_name) const {
    return textures.at(texture_name) != nullptr;
}

std::shared_ptr<sf::Texture> TextureManager::operator+=(const std::string &texture_name) {
